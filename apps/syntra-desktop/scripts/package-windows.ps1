...
if ([string]::IsNullOrWhiteSpace($OutputDir)) { $OutputDir = Join-Path $desktopRoot 'dist' }
...
Copy-Item $installer.FullName (Join-Path $OutputDir 'Syntra-Setup-0.4.0-x64.exe') -Force

$manifest = @(
  Get-FileHash (Join-Path $OutputDir 'Syntra.exe') -Algorithm SHA256
  Get-FileHash (Join-Path $OutputDir 'Syntra-Setup-0.4.0-x64.exe') -Algorithm SHA256
