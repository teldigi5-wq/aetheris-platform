...
public class LiveConnectorWriteExecutor {
    ...
    private static final Pattern SAFE_GITHUB_PERMISSION_HEADER =
            Pattern.compile("^[A-Za-z0-9_=,;:\\- .]{0,256}$");

    private static final String USER_AGENT =
            "Aetheris-Syntra-Connector/1.0";

    private static final String GITHUB_ACCEPT =
    ...
    public String execute(ConnectorActionEntity action) {
        ...
        if (credential.getProvider() != action.getProvider()) {
            throw new ConnectorActionBlockedException(
                    "OAuth credential provider does not match connector action"
            );
        }

        if (credential.getStatus() != ProviderCredentialStatus.ACTIVE) {
            throw new ConnectorActionBlockedException(
                    "OAuth credential must be ACTIVE before live connector writes"
            );
        }

        if (credential.getExpiresAt() != null
                && Instant.now().isAfter(credential.getExpiresAt())) {
            throw new ConnectorActionBlockedException(
                    "OAuth access token is expired; refresh is required"
            );
        }

        requireWriteScope(
                action.getActionKind(),
                credential.getScopesCsv()
        );
        ...
        return switch (action.getActionKind()) {
            case GMAIL_SEND_EMAIL ->
                    sendGmail(action, accessToken);

            case CALENDAR_CREATE_EVENT ->
                    createCalendarEvent(action, accessToken);

            case GITHUB_CREATE_ISSUE ->
                    createGithubIssue(action, accessToken);
        };
    }

    private String sendGmail(
        ...
        if (!EMAIL.matcher(recipient).matches()) {
            throw new ConnectorActionBlockedException(
                    "Gmail targetRef must be a valid recipient email address"
            );
        }
        ...
        if (messageId.isBlank()) {
            throw new ConnectorActionBlockedException(
                    "Gmail write response did not include a message id"
            );
        }
    ...
    private String createCalendarEvent(
        ...
        try {
            ...
            throw new ConnectorActionBlockedException(
                    "Calendar start/end values must be RFC3339 timestamps"
            );
        }

        if (!end.isAfter(start)) {
            throw new ConnectorActionBlockedException(
                    "Calendar end time must be after start time"
            );
        }
        ...
        payload.put(
                "description",
                "Created by Aetheris after explicit owner approval. Action "
                        + action.getId()
        );
        ...
        if (eventId.isBlank()) {
            throw new ConnectorActionBlockedException(
                    "Calendar write response did not include an event id"
            );
        }
    ...
    private String createGithubIssue(
            ConnectorActionEntity action,
            String accessToken
    ) {

        String repository =
                action.getTargetRef().trim();

        if (!GITHUB_REPOSITORY.matcher(repository).matches()) {
            throw new ConnectorActionBlockedException(
                    "GitHub targetRef must be owner/repository"
            );
        }

        String title =
                oneLine(action.getSummary());
        ...
        if (number.isBlank()) {
            throw new ConnectorActionBlockedException(
                    "GitHub write response did not include an issue reference"
            );
        }
    ...
    private JsonNode postJson(
        ...
        try {
        ...
        } catch (LiveProviderWriteException error) {
            ...
            throw new ConnectorActionBlockedException(
                    "Live provider write was interrupted"
            );

        } catch (IOException | IllegalArgumentException error) {

            throw new ConnectorActionBlockedException(
                    "Live provider write could not be completed"
            );
        }
    }

    private static String safeGithubPermissionHint(
        ...
        if (!SAFE_GITHUB_PERMISSION_HEADER.matcher(value).matches()) {
            return null;
        }
    ...
    private static void requireWriteScope(
            ConnectorActionKind kind,
            String scopesCsv
    ) {
        ...
        if (!allowed) {
            throw new ConnectorActionBlockedException(
                    "OAuth credential does not include the required provider write scope for "
                            + kind
            );
        }
    }
