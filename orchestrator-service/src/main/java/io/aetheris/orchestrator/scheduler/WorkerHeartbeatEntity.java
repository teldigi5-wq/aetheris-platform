...
public class WorkerHeartbeatEntity {
    ...
    public WorkerHeartbeatEntity(String workerId,int maxConcurrency,int activeLeases){this.workerId=workerId;heartbeat(maxConcurrency,activeLeases);}
    public String getWorkerId(){return workerId;} public int getMaxConcurrency(){return maxConcurrency;} public int getActiveLeases(){return activeLeases;} public Instant getLastHeartbeatAt(){return lastHeartbeatAt;} public Instant getUpdatedAt(){return updatedAt;}
    public void heartbeat(int maxConcurrency,int activeLeases){this.maxConcurrency=Math.max(1,Math.min(maxConcurrency,64));this.activeLeases=Math.max(0,Math.min(activeLeases,this.maxConcurrency));this.lastHeartbeatAt=Instant.now();this.updatedAt=this.lastHeartbeatAt;}
