...
public class VectorMemoryIndexService {
    ...
    @Transactional public int rebuild(){int count=0;for(KnowledgeNodeEntity node:nodes.findTop500ByOrderByUpdatedAtDesc()){index(node.getId());count++;}return count;}
    public List<VectorMemorySearchResult> search(String query,MemoryScope scope,String namespace,boolean includeProtected,int limit){float[] q=embedding.embed(query);if(norm(q)==0d)return lexicalFallback(query,scope,namespace,includeProtected,limit);List<VectorMemorySearchResult> out=new ArrayList<>();for(MemoryVectorEntity vector:vectors.findTop1000ByAdapterIdOrderByUpdatedAtDesc(embedding.id())){KnowledgeNodeEntity node=nodes.findById(vector.getKnowledgeNodeId()).orElse(null);if(node==null)continue;if(scope!=null&&node.getScope()!=scope)continue;if(namespace!=null&&!namespace.isBlank()&&!node.getNamespace().equalsIgnoreCase(namespace.trim()))continue;if(!includeProtected&&node.isProtectedData())continue;double score=cosine(q,vector.vector());if(score>0d)out.add(new VectorMemorySearchResult(node,score,"local vector cosine"));}out.sort(Comparator.comparingDouble(VectorMemorySearchResult::score).reversed());if(out.isEmpty())return lexicalFallback(query,scope,namespace,includeProtected,limit);return out.stream().limit(Math.max(1,Math.min(limit,50))).toList();}
